...
class LandmarkDetector:
    ...
    def draw_landmarks(image, detection_result):
        ...
        for idx in range(len(face_landmark)):
            ...
            annotated_image = cv2.circle(annotated_image, (int(
                landmark.x * width), int(landmark.y * height)), 1, (0, 0, 255), -1)
        return annotated_image
    ...
    def show_annotation(image, detection_result):
        ...
        annotated_image = LandmarkDetector.draw_landmarks(
            image, detection_result)
        cv2.imshow("Annotated Image", cv2.cvtColor(
...
class Morpher:
    ...
    def show_triangles(self):
        ...
        source_image = self.detector.draw_landmarks(
            source_image, self.source_landmarks)
        target_image = self.detector.draw_landmarks(
            target_image, self.target_landmarks)
        source_image = self.draw_triangles(
            source_image, self.source_points, self.source_triangles)
        target_image = self.draw_triangles(
            target_image, self.target_points, self.target_triangles)
        cv2.imshow("Source Triangles", cv2.cvtColor(
            source_image, cv2.COLOR_RGB2BGR))
        cv2.imshow("Target Triangles", cv2.cvtColor(
            target_image, cv2.COLOR_RGB2BGR))
        cv2.waitKey(0)
    ...
    def morph(self, alpha):
        ...
        for source_triangle, target_triangle in zip(self.source_triangles, self.target_triangles):
            ...
            morphed_triangle = (1-alpha)*source_triangle + \
                alpha*target_triangle
            ...
            M = cv2.getAffineTransform(
                source_triangle[:3], morphed_triangle[:3])
            N = cv2.getAffineTransform(
                target_triangle[:3], morphed_triangle[:3])
            # Warp the triangle
            warped_from_source = cv2.warpAffine(
                self.source, M, morphed_image.shape[:2])
            warped_from_target = cv2.warpAffine(
                self.target, N, morphed_image.shape[:2])
            mask = np.zeros(morphed_image.shape[:2], dtype=np.uint8)
            cv2.fillConvexPoly(
                mask, morphed_triangle[:3].astype(np.int32), 255)
            mask = np.repeat(mask[:, :, np.newaxis], 3, axis=2)
            morphed_image += warped_from_source * \
                (mask//255) * (1-alpha) + \
                warped_from_target * (mask//255) * alpha
